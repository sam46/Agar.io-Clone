class Organ {
    ...
    update() {
        ...
        if(this.applyPosEase){
            ...
            this.easeDist -= /*ease_spd*dt*ease_step*this.easeDist*/ Math.sqrt(this.maxspd*3.0 * this.easex ** 2 + this.maxspd*3.0 * this.easey ** 2);
...
class Player {
    ...
    constrain() {	// constrain organs movements
        ...
        for(var i = 0; i < this.organs.length-1; i++) {
            ...
            for(let j = i+1; j < this.organs.length; j++){
                ...
                const radSum = (org2.size+org1.size);		// sum of radii
    ...
    static interpolate(srcState, targetState, progress) {	
        const interpolatedState = new Player(srcState.pid);
        ...
        if(targetState == null)		
            ...
            for (let i = 0; i < srcState.organs.length; i++) {
                const tx = targetState.organs[i].x;
                const ty = targetState.organs[i].y;
                const tSize = targetState.organs[i].size;
                const sx = srcState.organs[i].x;
                const sy = srcState.organs[i].y;
                const sSize = srcState.organs[i].size;
...
function copyPlayer(src, target, preserveBounce){
    ...
    for (const curOrg of src.organs) {
        const temp = new Organ(curOrg.x, curOrg.y, curOrg.size,
